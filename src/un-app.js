...
export class UnApp extends LitElement {
	...
	renderHeader(headerData) {
		...
		return html`
			<un-header theme=${ifDefined(this.theme)} transfx="slide" transtime="15000">
		...
		return html`
			<un-menu-bar theme=${ifDefined(this.theme)}>
				...
				<un-drop-down slot="right" overlay openon="click" theme=${this.theme} lang=${this._lang}>
					...
					<div class="lang-switch">Deutsch</div>
				</un-drop-down>
			</un-menu-bar>
		`
	}
	...
	handleEvent(evt) {
		...
		if (evt.type === 'click' || evt.type === 'touchstart') {
			//this._logs.push(evt.type)
			let evtTargetZero = evt?.composedPath()[0]
			// <a> 
			if (evtTargetZero.matches('a') && evtTargetZero.href.includes(this.host)) {
				evt.preventDefault()
